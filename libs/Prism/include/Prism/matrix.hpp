...
class PRISM_EXPORT Matrix {
  public:
     ...
     */
    Matrix(size_t rows, size_t cols) : rows_(rows), cols_(cols), data_(rows, std::vector<T>(cols, 0)) {
        if (rows == 0 || cols == 0) {
    ...
    */
    ...
    public:
         ...
         */
        T& operator[](int col) { return m_row_vector.at(col); }
    };
    ...
    public:
         ...
         * @throws std::out_of_range if the column index is out of bounds.
         */
        const T& operator[](int col) const { return m_row_vector.at(col); }
    };
     ...
     * @return True if both matrices have the same dimensions and all elements are equal, false otherwise.
     */
    ...
    MatrixRow operator[](size_t i) {
        if (i >= rows_) throw std::out_of_range("Row index out of bounds.");
...
Matrix<T> Matrix<T>::operator*(const Matrix<T>& m) const {
    if (cols_ != m.rows_) {
        throw std::invalid_argument("Number of columns in the first matrix must match number of rows in the second matrix.");
    }
